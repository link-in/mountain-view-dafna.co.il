-- Simple Billing System for HOSTLY (without complex foreign keys)
...
VALUES
  ('free', 'free', 'חינם', 'תוכנית ניסיון', 0, 0, 1, 50, '["דשבורד בסיסי", "עד 50 הודעות WhatsApp"]'::jsonb),
  ('basic', 'basic', 'בסיסי', 'למתחילים', 99, 990, 1, 200, '["דשבורד מלא", "עד 200 הודעות WhatsApp", "תמיכה מהירה"]'::jsonb),
  ('pro', 'pro', 'מקצועי', 'לבעלי יחידות רציניים', 199, 1990, 5, 1000, '["דשבורד מתקדם", "עד 1000 הודעות WhatsApp", "עד 5 יחידות"]'::jsonb),
  ('enterprise', 'enterprise', 'ארגוני', 'לרשתות גדולות', 399, 3990, 999, 999999, '["הכל ללא הגבלה", "תמיכה 24/7"]'::jsonb)
...
-- 6. Helper Function: Increment Usage
...
-- 7. Helper Function: Check User Limits
...
BEGIN
  ...
  SELECT * INTO v_subscription FROM subscriptions WHERE user_id = p_user_id AND status IN ('active', 'trial');
...
-- 8. Give all existing users a trial subscription
...
-- Allow all (we handle auth in API)
CREATE POLICY "Allow all subscription_plans" ON subscription_plans FOR ALL USING (true);
...
-- Done!
SELECT 'Billing system created successfully!' AS result;
